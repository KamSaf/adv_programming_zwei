...
for mode in modes.keys():
    cnts = cv2.findContours(thresh.copy(), mode, cv2.CHAIN_APPROX_NONE)
    ...
    cv2.imshow(f"Image mode - {modes[mode]}", image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()


# Tryby RETR_EXTERNAL nie wykrywa uszkodzeń na kostkach, pozostae tak
